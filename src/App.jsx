...
// 1-2 사용자 말풍선 상단 고정 (html[data-userpin]) — 사용자 2026-09-16 "사용자 텍스트버블이 항상 상단 고정되는 인터랙션 적용이 필요"
const USERPINS = [
  { id: 'U1', label: 'U-1 시트 내려간 뒤 바로', desc: '시트가 내려가고 질문·답 말풍선이 뜨는 순간 곧바로 시작선(헤더 아래)으로 올라간다. 다음 턴의 생각 점은 이미 그 아래에서 시작한다. 지금은 다음 턴이 시작될 때야 올라가서 말풍선이 한동안 아래에 머물렀다' },
  { id: 'U2', label: 'U-2 한 박자 읽고', desc: '말풍선이 뜬 자리에서 0.6s 머물러 "무엇을 골랐는지" 읽게 한 다음 시작선으로 올라간다. 위치가 두 번 바뀌지만 선택 결과를 확인하는 순간이 생긴다' },
  { id: 'U3', label: 'U-3 시트 뒤에서 미리', desc: '시트가 아직 떠 있는 동안 말풍선이 뒤에서 시작선에 자리 잡고, 시트가 걷히면 이미 상단에 있다. 화면에서 움직이는 것이 시트 하나뿐이라 가장 조용하다. 요금제 결과(스텝 4)만은 팝업이 닫힌 뒤 올라간다' },
  { id: 'off', label: '없음 (기존)', desc: '비교용. 말풍선은 제자리에 뜨고, 다음 턴의 생각 점이 시작될 때 시작선으로 올라간다 — 지금 배포된 상태' },
]
...
// 1-2 신청서 시트가 연속될 때 키패드 (html[data-kbkeep]) — 사용자 2026-09-16 "키패드 올라온 상태로 연속되는 건 어떨까요? 바텀시트가 위아래로 반복 이동하는 게 줄어들 것"
const KBKEEPS = [
  { id: 'B1', label: 'B-1 키패드 유지', desc: '[다음] 을 눌러도 키패드는 내려가지 않는다. 시트 내용만 다음 장으로 바뀌고 커서가 새 입력칸으로 옮겨 간다. 시트가 키패드 위 자리에 계속 머물러 위아래 이동이 사라진다 — 제안하신 형태 그대로' },
  { id: 'B2', label: 'B-2 유지 + 한 박자', desc: '키패드는 그대로 두되, 시트가 바뀌는 동안 커서가 잠깐(0.3s) 사라졌다가 새 칸에 나타난다. 움직임 없이도 "장이 넘어갔다" 는 표시가 남아, 같은 자리에서 내용만 바뀌는 것이 어색하지 않다' },
  { id: 'B3', label: 'B-3 리턴 키로 넘김', desc: '키패드가 계속 떠 있으니 시트의 [다음] 대신 키패드의 ↵ 를 눌러 다음 장으로 간다. 손이 키패드를 떠나지 않아 실제 폼 입력 습관과 같다. 다만 [다음] 버튼의 역할이 마지막 장에서만 남아 두 가지 진행 방법이 공존한다' },
  { id: 'off', label: '없음 (기존)', desc: '비교용. [다음] → 키패드 내려감(0.3s) → 새 시트 → 키패드 다시 올라옴(T-2) — 지금 배포된 상태' },
]
const AUTOKBS = [
...
/* 1안 신청서 · 입력이 끝난 자리에 남는 컴포넌트 (html[data-findone], Figma ixGPs9 540:149303) — 사용자 2026-09-16 "일괄 변경 전에 시안 먼저" */
const FINDONES = [
  { id: 'K1', label: 'K-1 Figma 그대로', desc: '흰 카드(라운드 28 · 연한 테두리 · 패딩 22) 안에 라벨("주민등록번호")과 우측 밑줄 [재입력]. 입력 카드와 같은 흰 카드 언어라 "그 카드가 접혀 이렇게 남았다" 로 읽힌다. 값은 보이지 않아 가장 정돈돼 있다' },
  { id: 'K2', label: 'K-2 + 입력값', desc: '같은 카드에 라벨 아래 입력한 값이 회색 한 줄로 남는다 (주민등록번호는 900101-1****** 마스킹, 주소는 한 줄로 줄임). 무엇을 넣었는지 스크롤을 되감아 확인할 수 있어 [재입력] 을 누를지 판단하기 쉽다. 카드가 한 줄 높아진다' },
  { id: 'K3', label: 'K-3 + 완료 체크', desc: 'Figma 카드 왼쪽에 작은 체크 원이 붙는다. 기존 완료 선이 주던 "끝났다" 신호를 카드로 옮겨 진행감이 남는다. 다만 뒤에 오는 "번호 확인 완료" 체크 선과 기호가 겹친다' },
  { id: 'off', label: '없음 (이전)', desc: '비교용. "│ 주민등록번호 입력이 완료되었어요 ─ 수정하기" 회색 선 — K-1 확정 전 상태' },
]
...
export default function App() {
  ...
  const [kbkeepV, setKbkeepV] = useState(() => (LAB ? (Q0.get('kk') || CONFIRMED.kbkeep) : CONFIRMED.kbkeep))
  useEffect(() => { document.documentElement.dataset.kbkeep = kbkeepV }, [kbkeepV])
  ...
  const [findoneV, setFindoneV] = useState(() => (LAB ? (Q0.get('fd') || CONFIRMED.findone) : CONFIRMED.findone))
  useEffect(() => { document.documentElement.dataset.findone = findoneV }, [findoneV])
  ...
  const [userpinV, setUserpinV] = useState(() => (LAB ? (Q0.get('up') || CONFIRMED.userpin) : CONFIRMED.userpin))
  useEffect(() => { document.documentElement.dataset.userpin = userpinV }, [userpinV])
      ...
      {LAB && (
        <div className="lab-groups">
          ...
          <div className="variants v-userpin">
            ...
            <span className="vdesc">{USERPINS.find((v) => v.id === userpinV)?.desc}</span>
          </div>
          )}
          {SHOW_ALL && (
          <div className="variants">
          ...
          <div className="variants v-kbkeep">
            ...
            <span className="vdesc">{KBKEEPS.find((v) => v.id === kbkeepV)?.desc}</span>
          </div>
          ...
          <div className="variants v-findone">
            <b className="vtitle">시안 1-1 · 스텝 10~12 · 14 — 입력이 끝난 자리에 남는 컴포넌트 (Figma 540:149303) — K-1 확정</b>
            {FINDONES.map((v) => (
              <button key={v.id} aria-pressed={findoneV === v.id} onClick={() => { document.documentElement.dataset.findone = v.id; setFindoneV(v.id); userNav(); setReplay((n) => n + 1) }}>{v.label}</button>
            ))}
            <span className="vdesc">{FINDONES.find((v) => v.id === findoneV)?.desc}</span>
          </div>
